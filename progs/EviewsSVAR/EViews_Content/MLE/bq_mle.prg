...
subroutine local var_matrices(vector in_b, matrix ahat_svar, matrix alags, matrix f, matrix beta)
	...
	ahat_svar(1,2) = in_b(3)+in_b(5)+in_b(7)+in_b(9)+in_b(11)+in_b(13)+in_b(15)+in_b(17) 'This forces c(1,2) = 0
	ahat_svar(2,1) = in_b(1)

	'Now the lags. There are 32 lags to estimate.
	...
	'Count ends at 33 for the lagged coefficients+contemporaneous coeffcieints.  32 lagged coefficients plus 1 coefficient in the @inverse(A0) matrix.
	...
	beta(1,1) = in_b(36) '  Variance of the first structural error
	beta(2,2) = in_b(37) '  ...
...
pageselect Org_BQData

'logmode all

!m = 2				'Number of equations
!var_lags  =8		'Number of lags
!nx = 100				'Number of periods required in the impulse vector
...
!npars = 37      		'Total number of parameters to estimate in the system; (32 lags; 2 constants; 1 contemporaneous; 2 in the covariance matrix)
...
ylags.add dya(-1) u(-1) dya(-2) u(-2) dya(-3) u(-3) dya(-4) u(-4) dya(-5) u(-5) dya(-6) u(-6) dya(-7) u(-7) dya(-8) u(-8)
...
fe(!npars-1,1) = bq.@residcov(1,1)
fe(!npars,1) = bq.@residcov(2,2)
