...
subroutine local var_matrices(vector in_b, matrix ahat_svar, matrix alags, matrix f, matrix beta)
	...
	ahat_svar(2,3) = in_b(21) + in_b(25) + in_b(29) 'no permanent effect of interest rate shocks to output
	ahat_svar(2,4) = in_b(22) + in_b(26) + in_b(30)' no permanent effect of demand shocks to output; these are the lageed coefficients on the CPI ((A1(2,4), A2(2,4), A3(2,4))
	...
	ahat_svar(4,3) = (ahat_svar(2,3)/ahat_svar(2,4)) 'This constraint ensures that the short-run effect is 0.

	'show ahat_svar

	'Now the lags. There are 48 lag coefficients to estimate: 7 to 54 
	...
	'Count ends at 54 for the lagged coefficients+contemporaneous coeffcieints.  48 lagged coefficients plus 6 coefficients in the A matrix.
	...
	f(1,1) = in_b(55) 'Constant terms
	f(2,1) = in_b(56)
	f(3,1) = in_b(57)
	f(4,1) = in_b(58)

	f(1,2) = in_b(59) 'Trend terms
	f(2,2) = in_b(60)
	f(3,2) = in_b(61)
	f(4,2) = in_b(62)
	...
	beta(1,1) = in_b(63) '  Variance of the first structural error
	beta(2,2) = in_b(64) '  ...
	beta(3,3) = in_b(65)
	beta(4,4) = in_b(66)
...
!npars = 66	      	'Total number of parameters to estimate in the system; (47 lags; 8 (constants+trend); 7 contemporaneous; and 4 in the covariance matrix)
...
group vdata.add dpoil dusgdp usint duscpi 

group xdata.add c (time+5)
...
ylags.add dpoil(-1) dusgdp(-1) usint(-1) duscpi(-1) dpoil(-2) dusgdp(-2) usint(-2) duscpi(-2) dpoil(-3) dusgdp(-3) usint(-3) duscpi(-3)

'Notice the ordering of the lags: all of -1 first; followed by all of -2 second, and so on.

stom(ylags, ym_lags)

var peersman.ls 1 3 dpoil dusgdp usint duscpi  @ c (time+5) 'To be consistent with RATS

show peersman

'The constrained system imposes two constraints....

vector(!npars) fe = 0.0

matrix sigma=@identity(!m) 'This will eventually contain the variances of the structural errors

peersman_sys.fiml(conv=1e-10,rcov=diag)

'Create FIML ahat_matrix

matrix(4,4) fiml_ahat = @identity(!m)

fiml_ahat(2,1) = -c(57)
fiml_ahat(2,3) = (c(21)+c(22)+c(23))
fiml_ahat(2,4) = (c(24)+c(25)+c(26))
...
fiml_ahat(4,1) = -c(61)
fiml_ahat(4,2) = -c(62)
fiml_ahat(4,3) = ((C(21)+C(22)+C(23))/(C(24)+C(25)+C(26)))
...
fe(63,1) = peersman_sys.@residcov(1,1)
fe(64,1) = peersman_sys.@residcov(2,2)
fe(65,1) = peersman_sys.@residcov(3,3)
fe(66,1) = peersman_sys.@residcov(4,4)

matrix fe_starting_values = fe

show fe

matrix rcov = peersman_sys.@residcov

scalar xx_1 = peersman_sys.@se(1)^2
scalar xx_2 = peersman_sys.@se(2)^2
scalar xx_3 = peersman_sys.@se(3)^2
scalar xx_4 = peersman_sys.@se(4)^2
