...
subroutine local var_matrices(vector in_b, matrix ahat_svar, matrix alags, matrix f, matrix beta)
	...
	ahat_svar(2,4) = -in_b(2)

	ahat_svar(3,1) = in_b(3)
	ahat_svar(3,2) = in_b(4)
	ahat_svar(3,4) = in_b(5)

	ahat_svar(4,1) = in_b(6)
	ahat_svar(4,2) = 7.582058802117931'ahat_svar(1,2)/ahat_svar(1,4) '
	ahat_svar(4,3) = ahat_svar(1,3)/ahat_svar(1,4) 
...
vector fe = fe_starting_values 'These starting values seem to work OK

matrix sigma=@identity(!m) 'This will eventually contain the variances of the structural errors

fe(!npars-3,1) = galiqje.@residcov(1,1)
fe(!npars-2,1) = galiqje.@residcov(2,2)
fe(!npars-1,1) = galiqje.@residcov(3,3)
fe(!npars,1) = galiqje.@residcov(4,4)

matrix fe_starting_values = fe 

matrix rcov = galiqje.@residcov

scalar xx_1 = galiqje.@se(1)^2
scalar xx_2 = galiqje.@se(2)^2
scalar xx_3 = galiqje.@se(3)^2
scalar xx_4 = galiqje.@se(4)^2
