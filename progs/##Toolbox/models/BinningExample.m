function [ENDO,EXOG,opt] = BinningExample(opt)
% =======================================================================
% Replication of: Binning (2013) - Smets and Wouters (2001) data
% data order:
...
[xlsdata, xlstext] = xlsread('./datasets/Smets_Wouters_data.xlsx');
ENDO = xlsdata(74:end,[2,5:8]);
EXOG=[];
opt.dates = xlsdata(74:end,1);
opt.dates_freq ='Q';
opt.vnames = xlstext([1,4:7]);
opt.vnames_ex = [];
...
% Sign restrictions on impact
             %mp  %ad %as %ls
opt.Rsign  = [+1  +1  -1  -1 nan; %interest rates
              -1  +1  +1  +1 nan; %gdp
...
% Sign restrictions at horizon h
             %mp  %ad %as %ls
opt.Rsignh{1}  = [+1  +1  -1  -1 nan; %interest rates
                  -1  +1  +1  +1 nan; %gdp
                  nan nan nan +1 nan; %hours
                  -1  +1  -1  -1 nan; %inflation
                  nan nan nan -1 nan; %wages
